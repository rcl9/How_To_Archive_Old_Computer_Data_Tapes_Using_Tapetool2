...
Overall I found that the data on two tapes sounded (to me) fairly consistent and okay. However, one original tape (which may not have been made by me) was choppy while another one tended to have issue for the first few files. I'd quickly find out that this was true when trying to turn the audio back into error-free digital data. 
...
In both of these examples you will notice that I am using a bandpass filter and noise filtering. Through my experience I found that the bandpass filter was absolutely necessary but that the noise filtering most often lead to failed de-blocking. Hence, using IF/ELSE statements in the script file, I set up another code path which would drop the using of noise filtering via these two defines:
...
[Storing and Loading Exidy Sorcere Programs From Cassette Tapes](https://web.archive.org/web/20260211031055/https://www.trailingedge.com/exidy/exbasic11.html)
